...
| Persist / resume a run's full state | `RunnableStrategy::{save_state, restore_state, drive_resumable}` + `RunState` (public fields, `PartialEq`, serde); `backtest::{run_iteration_resumable, apply_closeout}`. Python mirrors it as the `RunState` pyclass — **not** a JSON `str` — and reaches the account half alone via `PaperWallet.{snapshot_state,restore_state}`. See ARCHITECTURE *Run resuming* | `src/spec/{runnable,backtest}.rs`, `src/backtest.rs`, `python/src/{spec,strategy}.rs` |
| Make a resumed run **re-size its book now** | `Closeout::Rebalance { hold }` → `backtest::run_rebalancing` → `Strategy::force_rebalance` (one-shot latch, armed around the **final** bar's `trade`, never serialized). Queues through `Wallet::set` — **not** `settle_position`: a rebalance isn't terminal. **Bar-lessly it settles**, via `Wallet::settle_pending` — there is no next `open` to queue for, and a queued move would fill on a venue now and on paper a bar later. Python `run_resumable(rebalance=True)`, CLI `--rebalance`. See ARCHITECTURE *Rebalance on demand* | `src/backtest.rs`, `src/strategy.rs`, `src/strategies/mod.rs` |
| …with **no bar to arm around** — an operator saying *now*, between two closes | `backtest::rebalance_now`, picked by `drive_over` on an empty stream. Same arm/`trade`/clear, fired against the marks the wallet already carries; consumes no bar, advances no clock, adds no equity point. An unready strategy is an **`Err`**, not a shrug: with no bars there is no report to read *delivered* off | `src/backtest.rs`, `src/spec/runnable.rs` |
...
| How much of a multi-symbol universe ever shares a snapshot | `overlap::{measure, measure_universe}` → `Overlap<K>` (`is_fragmented()` / `summary()`) in the **library** (Python: `ta.measure_overlap`); `cli::overlap::warn_if_fragmented` is the console half. Measures **observed co-occurrence**, never per-symbol stamp signatures. Joining on the trading *date* is rejected permanently — it manufactures cross-timezone lookahead (see TODO) | `src/overlap.rs`, `src/cli/overlap.rs` |
...
| Fetch any series (candles *or* price-less) | `SeriesSource::atoms(...)` — `Binance`, `BinanceVision` (spot *and* USDⓈ-M futures), `BinanceFutures` (the archive's live twin, one shared schema), `Okx`, `Kraken`, `Coinbase`, `Yahoo`, `CoinGecko`; the CLI adds `file:`. `schema()` = the fixed overlay schema when known before the fetch | `src/sources/mod.rs`, `src/cli/csv_source.rs` |
| Fold off-cadence side-channel samples onto the bar | `sources::bucket::{Fold, Aggregation}` — accrual sums, level keeps the newest **by sample timestamp**. Shared by both perpetual providers; don't hand-roll a second bucketing | `src/sources/bucket.rs` |
