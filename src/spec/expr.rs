...
use fugazi_derive::{SpecGrammar, SpecRaw};
...
/// tagged shapes into the map shape `NodeSpecRaw` expects; `NodeSpecRaw` —
/// **generated by `#[derive(SpecRaw)]`**, never written by hand — carries the
/// derived externally-tagged deserialization logic (every variant and field
/// of this enum with only the `#[serde(...)]` attributes carried over), plus
/// the mechanical `From<NodeSpecRaw> for NodeSpec`.
#[derive(Debug, Clone, Deserialize, SpecGrammar, SpecRaw)]
#[serde(try_from = "serde_norway::Value")]
#[grammar(group = "node")]
pub enum NodeSpec {
