...
| A document's free-form `meta:` (open schema, never interpreted) | `spec::meta::Meta` + a `meta` field on every document type; read via `StrategySpec::meta()` / `StrategyRef::meta()` / `CostConfig::meta()` / `overlay::columns_and_meta_from_{value,yaml}`; Python `StrategySpec.meta`. **Don't relax `deny_unknown_fields` instead** — the typo guard is the point | `src/spec/meta.rs` |
