...
- **Weight-share indicators (adaptive weighting).** `PortfolioBuilder::weight_shares(Vec<Box<dyn Indicator<Input=Snapshot<Sym>, Output=Real>>>)` installs one real-valued chain per child (in `.add(...)` order). Advanced every bar in `Portfolio::update`; at each rebalance-fire, the portfolio reads each's `.value().unwrap_or(0.0).max(0.0)` and normalizes `w_i = N_i / Σ N_j` — that vector wins over `WeightPolicy::weights`. When every share reads `0` (still warming, or genuinely zero) the policy is used as fallback so a rebalance direction is always defined. YAML surface: `weights: !indicator <template>` — a `SpecTemplate<ExprSpec>` instantiated once per child at build with `!arg CHILD_NAME` (always available) and `!arg SYM` (single-asset children only — a template using `!arg SYM` against a basket / multi / pairs child errors at build). Per-child book plumbing is wired: each template instance reads against *that* child's own `Book<String>` (grabbed via `DynSingleStrategy::book` / `DynPairsStrategy::book` / `DynBasketStrategy::book` / `DynMultiAssetStrategy::book`), so `!drawdown`, `!return_per_bar`, `!trade_return` inside a weight template read from the child's real state. Adaptive weighting (inverse-vol, Kelly-fraction, drawdown-throttled) is just a matter of writing the right expression.
...
| Portfolio adaptive weighting (per-child indicator) | `PortfolioBuilder::weight_shares(Vec<Box<dyn Indicator<Input=Snapshot<Sym>, Output=Real>>>)`; YAML `weights: !indicator <template>` instantiated per-child with `!arg SYM` (single-asset) / `!arg CHILD_NAME`; reads against each child's own Book | `src/portfolio/mod.rs`, `src/cli/spec/portfolio.rs` |
