...
- **Position-anchored sources** (`indicators/entry.rs`) read the live position rather than the candle stream: `Entry` (the entry price), `PeakSinceEntry`/`TroughSinceEntry` (the running high/low since entry). All read from a shared `EntryAnchor` (`Rc<Cell<Option<Real>>>`) a strategy owns and arms on entry; `None` while flat. They exist to express stop-loss / take-profit **levels** (e.g. `Entry::new(anchor).sub(Atr::new(14).mul(...))` for a fixed stop, `PeakSinceEntry` for a trailing one) and are only meaningful inside a strategy that arms the anchor.
...
- `Order<Sym>` (`{ symbol, side, units }`); `Order::from_delta(symbol, delta)` builds the buy/sell for a position change (`None` within `DEFAULT_EPSILON`).
