...
`tests/talib_validation.rs` checks arcana's indicators against
...
- `aapl_monthly.csv` — committed OHLCV input (`date,open,high,low,close,volume`).
  ...
  valid because arcana **and** TA-Lib consume this exact file. Replace it with
...
mamba run -n arcana-talib python3 tools/gen_talib_fixtures.py
...
to `activate` it first (equivalently: `conda activate arcana-talib` then run
`python3 tools/gen_talib_fixtures.py`). If your shell wrapper can't run
`mamba run`, call the env's interpreter directly:
`"$(mamba env list | awk '/arcana-talib/{print $NF}')"/bin/python3
