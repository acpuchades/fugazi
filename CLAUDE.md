...
**`BasketStrategy<Sym>`** (`strategies/basket.rs`) — N-symbol cross-sectional. Reads whole `Snapshot<Sym>` each bar: scores every symbol via per-symbol scoring source, applies a [`Selection`] trait impl (`fn pick(&self, scores: &HashMap<Sym, Real>) -> HashMap<Sym, Side>`) — one built-in impl per rule plus a blanket `Fn` impl for closures.

- **Selection.** Trait: `strategies::basket::Selection<Sym>`. Built-in impls: `TopBottom { longs, shorts }` / `Threshold { long_min, short_max }` / `Quantile { long_q, short_q }`. Builders `.top_bottom(l, s)` / `.threshold(min, max)` / `.quantile(lq, sq)` install those. `.selection(impl)` = general seam — accepts any `Selection<Sym>` impl (or a `Fn(&HashMap<Sym, Real>) -> HashMap<Sym, Side>` closure, via the blanket impl). Free functions `strategies::basket::{top_bottom, threshold, quantile}` remain callable for direct use.
...
- **State.** `PortfolioInner<Sym>` owns `subs: Vec<PaperWallet<Sym>>` + id-tracking tables + optional `TradingCosts` cloned per sub (see the `Clone` impl on `Box<dyn CommissionModel/SpreadModel/SlippageModel>` via `clone_box`). Portfolio itself owns children, policy, rebalance signal, bars_seen, and an **aggregate `Book<Sym>`** — marked to market on each `update()` from `Σ sub.equity()` via `Book::mark_equity`, exposed via `Portfolio::book()`, and passed by `PortfolioSpec::build` as the `portfolio_book` build argument to each per-child weight-share template so `source: !portfolio_book` inside a template resolves to it.
...
- **Not shipped** (yet): inverse-vol / performance-weighted `WeightPolicy` built-ins, Python bindings.
...
**Bound — all four strategy shapes + `run`.** `PyStrategy` mirrors `SingleAssetStrategy` — `Strategy(sym).long_on(...).short_on(...).position_sizing(src).run(wallet, candles)` → `PyRunReport`. `AtomLift` bridges candle-rooted Python signals to snapshot-rooted strategy layer. `PyPairsStrategy` / `PyMultiAssetStrategy` / `PyBasketStrategy` mirror their Rust siblings and drive over a sequence of snapshots (`.run(wallet, snapshots)`). Multi / basket factory slots are per-symbol **Python callables** (`sym -> Signal` / `sym -> Indicator`) converted via `signal_factory_from_callable` / `source_factory_from_callable` — the closure captures the `Py<PyAny>` callable and calls it once per symbol under the GIL (`Python::attach`); a factory that raises or returns the wrong type surfaces as a Python exception via pyo3's panic bridge. **Not bound** (don't add without asking): position-anchored protective levels (the per-leg `Position` isn't passed to Python factories), `BasketStrategy::selection(closure)` escape hatch (top-bottom / threshold / quantile only).

**Intentionally not bound**: `Strategy` trait as subclassable, `run_iteration`/`evaluate*`, trailing risk indicators, the CLI, `Wallet` trait (only `PaperWallet`), `Portfolio`, Rust-internal types (`Position`, `PositionField`, `Ack`, `OrderId`, `Reference`, `Units`).
...
| Cross-sectional rank → `Side` | Trait `strategies::basket::Selection<Sym>` with built-in impls `TopBottom` / `Threshold` / `Quantile`; free functions `top_bottom` / `threshold` / `quantile` still available for direct use. `BasketStrategy::selection(impl)` installs any impl (or a closure via the blanket `Fn` impl) | `src/strategies/basket.rs` |
| Declared basket universe (strict vs. lax) | `BasketStrategy::{all_of, any_of, universe}` on the Rust builder — `universe` takes any `Box<dyn Universe<Sym>>`; the trait lives at `strategies::basket::Universe` with built-in impls `Floating` / `AllOf<Sym>` / `AnyOf<Sym>`. YAML `universe: !all_of [...] \| !any_of [...]` on `BasketStrategySpec` builds the built-in impls | `src/strategies/basket.rs`, `src/cli/spec/basket.rs` |
...
| Portfolio YAML surface | `PortfolioSpec` (fields: `children`, `weights: Option<SpecTemplate<ExprSpec>>`, `rebalance_on`) + `PortfolioChildSpec` + `PortfolioChildStrategy`; `portfolio:` prefix on `run`/`optimize`; `backtest::{measured_report_portfolio, evaluate_portfolio, evaluate_windowed_portfolio, run_iteration_portfolio}`; runner `run::run_portfolio` | `src/cli/spec/portfolio.rs`, `src/cli/{backtest,run,optimize,input,main}.rs` |
...
| Portfolio adaptive weighting (per-child indicator) | `PortfolioBuilder::weight_shares(Vec<Box<dyn Indicator<Input=Snapshot<Sym>, Output=Real>>>)`; YAML `weights:` is a bare `SpecTemplate<ExprSpec>` (no `WeightPolicySpec` enum, no `!indicator` wrapper) instantiated per-child with `!arg SYM` (single-asset) / `!arg CHILD_NAME` / `!arg CHILD_INDEX`. Book source is explicit on each book-reading node — bare reads the child's own book (`strategy_book` build arg), `source: !portfolio_book` reads the aggregate | `src/portfolio/mod.rs`, `src/cli/spec/portfolio.rs` |
| Sugar tag rewrites (all lower to `!value` at load — no special-cased constant indicators) | (a) portfolio `weights:` — `rewrite_weights_sugar` lowers `!fixed [w0, w1, ...]` → `!value [w0, w1, ...]` and `!equal_weight` → `!value 1.0` before wrapping in `SpecTemplate<ExprSpec>`. (b) sizing `!equal_weight <N>` — `rewrite_sugar_tags` inside `ExprSpec::try_from` lowers to `!value <1/N>` before typed parse. | `src/cli/spec/portfolio.rs`, `src/cli/spec/expr.rs` |
| Per-child indexing of a list literal in weight expressions | `ExprSpec::Value(ValueLit::List(Vec<Real>))` — a `!value <list>` payload. `PortfolioSpec::build` runs `rewrite_value_list_by_index` per child before typed parse, rewriting `!value <list>` → `!value <list[i]>` | `src/cli/spec/expr.rs`, `src/cli/spec/portfolio.rs` |
| Aggregate portfolio Book (equity / peak / drawdown / return marked on `update`) | `Portfolio::book()` returns a shared handle to the aggregate `Book<Sym>`, marked to market via `Book::mark_equity` from `Σ sub.equity()` on each `Portfolio::update`; passed by `PortfolioSpec::build` as the `portfolio_book` build arg to each per-child weight-share template | `src/portfolio/mod.rs`, `src/indicators/book.rs`, `src/cli/spec/portfolio.rs` |
| Select which book a book-reading node reads (portfolio weight scope) | `source:` field on every book-reading node — `!strategy_book` (default: the enclosing strategy's own book) or `!portfolio_book` (the aggregate; only meaningful in a portfolio weight template — panics at build elsewhere). Both are build-time source-selectors: they carry no runtime value and bare use panics. Resolution happens via `resolve_book_source` inside `ExprSpec::build` | `src/cli/spec/expr.rs` |
| Book field leaves (composable inside any expression) | `!equity`, `!equity_peak`, `!drawdown`, `!return_per_bar`, `!trade_pnl`, `!trade_return` — each takes optional `source:` (defaults to `!strategy_book`). Book-anchored sizing recipes (`!drawdown_throttle` / `!equity_vol_target` / `!fractional_kelly`) take the same `source:` field | `src/cli/spec/expr.rs` |
