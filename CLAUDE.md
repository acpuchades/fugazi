...
- **Not shipped** (yet): dynamic rebalance (`rebalance_on` gate + `ReweightMode` — v1 is init-only, weights drift with per-child P&L), inverse-vol / performance-weighted policies, YAML surface (portfolio spec will lean on `!import` with inline params for name isolation, see CLI section), Python bindings.
