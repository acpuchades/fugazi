...
| `!fixed`/`!equal_weight` sugar (rewritten to `!value` at load) | `rewrite_weights_sugar` in the `weights:` deserializer lowers `!fixed [w0, w1, ...]` → `!value [w0, w1, ...]` and `!equal_weight` → `!value 1.0` before wrapping in the `SpecTemplate<ExprSpec>` | `src/cli/spec/portfolio.rs` |
