...
- **State.** `PortfolioInner<Sym>` owns `subs: Vec<PaperWallet<Sym>>` + id-tracking tables + optional `TradingCosts` cloned per sub (see the `Clone` impl on `Box<dyn CommissionModel/SpreadModel/SlippageModel>` via `clone_box`). Portfolio itself owns children, policy, rebalance signal, bars_seen, and an **aggregate `Book<Sym>`** — marked to market on each `update()` from `Σ sub.equity()` via `Book::mark_equity`, exposed via `Portfolio::book()`, and used by `PortfolioSpec::build` as the default anchor for weight-share templates (with each per-child clone linked to the child's own book via `Book::linked_to` so `!at_child` can walk to it).
...
| Aggregate portfolio Book (equity / peak / drawdown / return marked on `update`) | `Portfolio::book()` returns a shared handle to the aggregate `Book<Sym>`, marked to market via `Book::mark_equity` from `Σ sub.equity()` on each `Portfolio::update`; used by `PortfolioSpec::build` as the default anchor for weight-share templates | `src/portfolio/mod.rs`, `src/indicators/book.rs`, `src/cli/spec/portfolio.rs` |
| Reach per-child book from a portfolio-scoped weight template | `!at_child { source }` — rebuilds `source` against `book.linked()` (the child book paired with the aggregate at PortfolioSpec build time); pairs with `!drawdown`/`!equity`/`!equity_peak`/`!return_per_bar`/`!drawdown_throttle`/`!equity_vol_target`/`!fractional_kelly` inside for per-child reads. Panics at build if used where no linked book is wired | `src/cli/spec/expr.rs`, `src/cli/spec/portfolio.rs` |
| Book field leaves (composable inside any expression, including inside `!at_child`) | `!equity`, `!equity_peak`, `!drawdown`, `!return_per_bar`, `!trade_pnl`, `!trade_return` — unit-variant tags on `ExprSpec` reading the enclosing scope's `Book` | `src/cli/spec/expr.rs` |
...
| Pair a `Book` with another for cross-scope reads | `Book::linked_to(other) -> Self` + `Book::linked() -> Option<&Book<Sym>>`; the link is `Arc`-shared so the graph is cheap. Reset preserves the link. Used by `PortfolioSpec::build` to attach each per-child book to a per-child clone of the aggregate so `!at_child` can walk to it | `src/indicators/book.rs` |
