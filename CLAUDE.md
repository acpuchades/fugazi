...
- **`WeightPolicy` (`portfolio::policy`).** `trait WeightPolicy { observe(&mut, &[ChildSample]); weights(&self, n) -> Vec<Real>; warm_up_period() -> usize; reset(&mut) }`. Two built-ins ship: **`Fixed(Vec<Real>)`** and **`EqualWeight`**. `weights(n)` is queried once at build to split the initial cash budget across children; the observe hook is called every bar and reserved for a future dynamic-rebalance path.
- **State.** `PortfolioInner<Sym>` owns `subs: Vec<PaperWallet<Sym>>` + id-tracking tables + optional `TradingCosts` cloned per sub (see the `Clone` impl on `Box<dyn CommissionModel/SpreadModel/SlippageModel>` via `clone_box`). Portfolio itself owns children, policy, bars_seen, and the seed allocations captured at build.
- **Readiness.** `is_ready() = bars_seen >= policy.warm_up_period() && every child is ready` — the safe-defaults rule aggregated over every leg.
- **YAML.** `portfolio:` prefix + `PortfolioSpec` (`src/cli/spec/portfolio.rs`) mirror the composite. `children:` is an ordered list of `{ name, strategy }` slots where `strategy:` accepts any of the four shapes (single / pairs / basket / multi) routed by distinctive top-level key (`left`+`right` → pairs, `selection` → basket, `symbol` → single, otherwise multi; a preset tag like `!ma_crossover` also picks single). `weights:` is optional (`!fixed [w1, …]` or `!equal_weight`, default equal). The natural way to reuse one child spec N times with distinct params is `!import { path, params }`, since it isolates each import's `!param` names from the outer table. Wired into `run.rs` (`run_portfolio`) and `optimize.rs` (`Portfolio` arm on `run_multi_symbol` + walkforward — `DynPortfolio` captures per-child stable/warm at build for the layout probe). Costs (v1): the composite wallet takes one uniform `TradingCosts` bundle, so `--costs` is honored **unscoped** only — scoped `SYM:…` overrides are silently ignored at the portfolio boundary.
- **Not shipped** (yet): dynamic rebalance (`rebalance_on` gate + `ReweightMode` — v1 is init-only, weights drift with per-child P&L), inverse-vol / performance-weighted policies, Python bindings, per-symbol `--costs` scoping through the composite wallet.
...
| Composite Strategy over N heterogeneous children with per-child sub-wallets | `Portfolio::builder().add(name, strategy).weights(policy).costs(costs).build()`; `portfolio.wallet_view()` returns the aggregate `PortfolioWallet<Sym>` for `backtest::run` | `src/portfolio/mod.rs`, `src/portfolio/wallet.rs` |
| Portfolio YAML surface | `PortfolioSpec` + `PortfolioChildSpec` + `PortfolioChildStrategy` + `WeightPolicySpec`; `portfolio:` prefix on `run`/`optimize`; `backtest::{measured_report_portfolio, evaluate_portfolio, evaluate_windowed_portfolio, run_iteration_portfolio}`; runner `run::run_portfolio` | `src/cli/spec/portfolio.rs`, `src/cli/{backtest,run,optimize,input,main}.rs` |
| Portfolio weight policies (init cash split) | `portfolio::policy::{WeightPolicy, Fixed, EqualWeight, ChildSample}` | `src/portfolio/policy.rs` |
