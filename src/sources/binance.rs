...
impl CandleSource for Binance {
    ...
    fn candles(
        ...
        async move {
            ...
            let mut out: Vec<TimedCandle> = Vec::new();
            ...
            while cursor < until_ms {
                ...
                let last_open = out.last().expect("just pushed at least one row").time.0;
...
/// Extract one kline row from the API response.
fn decode_row(row: &serde_json::Value) -> Result<TimedCandle, SourceError> {
    let arr = row
        .as_array()
    ...
    Ok(TimedCandle {
        time: Timestamp(open_time),
        candle: Candle::new(open, high, low, close, volume),
    })
}
...
mod tests {
    ...
    #[test]
    fn decode_row_parses_string_numbers() {
        ...
        let tc = decode_row(&row).unwrap();
        assert_eq!(tc.time, Timestamp(1_700_000_000_000));
        assert_eq!(tc.candle.open, 27000.50);
        assert_eq!(tc.candle.high, 27100.00);
        assert_eq!(tc.candle.low, 26950.10);
        assert_eq!(tc.candle.close, 27050.75);
        assert_eq!(tc.candle.volume, 12.345);
    }

    #[test]
    fn decode_row_tolerates_bare_numbers() {
        // A mock server may return numbers un-stringified; accept both.
        let row = serde_json::json!([1_700_000_000_000_i64, 1.0, 2.0, 0.5, 1.5, 10.0]);
        let tc = decode_row(&row).unwrap();
        assert_eq!(tc.candle.close, 1.5);
    }
}
