...
- [`get`](#get) — fetch OHLCV bars from a remote provider (`binance`,
  `binance-futures`, `binance-vision`, `binance-vision-futures`, `okx`, `kraken`,
  `coinbase`, `yfinance`, `cg`) or re-process a local file (`file:PATH`) into a `run`-ready file,
...
| `--leverage <X>` | How far a fractional `sizing:` **deploys**, as a multiple of equity. Default `1` — the document is taken at face value. **This is the knob that trades an unedited strategy levered:** `--leverage 3` makes the same `sizing: 1.0` target 3x equity, and raises `--max-gross` to match so the request is not simply fitted back down. It multiplies what the rule *asks for*, which `--max-gross` structurally cannot — a ceiling can only ever stop a request, never enlarge one. It scales both fractional sizings (`value_frac`, `funds_frac`) and neither absolute one (`units`, `position_frac`): a named unit count is a specific intent. It scales a **risk-denominated** rule too — a `!vol_target` document at `--leverage 3` holds three times its target vol, which is the honest reading of asking for 3x. If you want the target left alone and only its truncation lifted, raise `--max-gross` on its own instead. |
| `--max-gross <X>` | Most gross notional the account may hold, as a multiple of equity. **Defaults to `--leverage`**, so raising deployment raises the ceiling with it and you pass this only to hold the two apart (`--leverage 3 --max-gross 5` deploys 3x but tolerates a book drifting to 5x on marks). No fill may leave `Σ \|position\| × price` above `X × equity`. **The one bound both sides share** — a buy is limited by the cash it spends, but a short *credits* cash, so without this `sizing: 3.0` took 1x long and 3x short under one spec value. For a long-only book at `1` it is the cash rule restated, so an unlevered long backtest is unchanged. Raise it to model a margined account (cash may then go negative — that is what borrowing is). A `sizing:` above the cap is scaled to it and the ask recorded in `fills.csv`'s `requested_units`; an explicit unit target is refused instead. Exits are always exempt. Carry on the borrowed part is charged only if you configure it — see `--margin-rate`, `--maintenance-margin` and the `carry` cost leg in [COSTS.md](COSTS.md). |
...
| `--margin-rate <RATE>` | Annualized interest on a negative cash balance. Same meaning as `run --margin-rate`. Default `0`. |
| `--maintenance-margin <RATIO>` | Maintenance-margin ratio. Same meaning as `run --maintenance-margin`. Off by default. |
| `--max-gross <X>` | Most gross notional each grid point's account may hold, as a multiple of equity. Same meaning as `run --max-gross`; defaults to `--leverage`. |
| `--leverage <X>` | How far a fractional `sizing:` deploys, as a multiple of equity. Same meaning as `run --leverage`, applied to every grid point. Default `1`. |
...
**Reading the result.** Two CSV columns are appended, suffixed onto the
`--best-by` metric the same way every other flag-gated column in this file is:
`<metric>_smoothed` (the neighbourhood average the rows are ranked by, in the
metric's native orientation) and `<metric>_support`. So a `-w` sweep smoothed on
Sharpe writes `risk_adjusted.sharpe_mean`, `risk_adjusted.sharpe_std`,
`risk_adjusted.sharpe_smoothed`, `risk_adjusted.sharpe_support`. Caveat, and it
predates smoothing: under `-k` the ranking key is `mean − K·std`, which has no
column of its own, so `_smoothed` is the neighbourhood average of a quantity the
CSV doesn't otherwise show — the console `best` block prints it as `score`.
...
means a ticker carrying `=` or `:` needs no escaping —
`fugazi get 'yfinance:EURUSD=X[1d]'`, `fugazi get 'binance-vision:BTC/USDT:USDT[1d]'`.
...
| `binance-futures` | `binance-futures:BTCUSDT[1d,1h]` | Binance USDⓈ-M perpetual klines, live from `fapi` — **OHLCV plus** the same side channels `binance-vision-futures` publishes (`funding_rate`, `premium_index`, `open_interest`, `open_interest_value`, the long/short ratios), on one schema with it, so the two concatenate. Current to the bar forming now, where the archive lags ~2 days; in exchange `open_interest` and the four ratios reach back **30 days only** (that is what `fapi` serves), while bars, funding and premium index reach back to listing. Funding settlements inside one bar are **summed**, the rest are levels — as in the archive, and each level lands on the bar it measured rather than the next (the snapshot endpoints stamp a window by its close, and the fetch un-stamps it). Two columns agree with the archive only approximately: `fapi` rounds the three account ratios to four decimals, and `taker_long_short_ratio` is the *whole bar's* taker flow here against the archive's last 5-minute sample within it. Frequencies: `1m`, `3m`, `5m`, `15m`, `30m`, `1h`, `2h`, `4h`, `6h`, `8h`, `12h`, `1d`, `3d`, `1w`, `1M`; the statistics endpoints speak a coarser vocabulary and are sampled at the finest period fitting inside the bar. |
...
- **Scope** (optional): `SYMBOL[FREQ]:`, `SYMBOL:`, or `[FREQ]:` — the
  ...
  fetch. Cells outside the scope render blank. The scope ends at the first
  `:`, so a symbol containing one escapes it as `\:`
  (`-x 'BTC/USDT\:USDT[1d]:r=!rsi { period: 2 }'`). A symbol containing `=`
  needs no escape — `-x 'EURUSD=X[1d]:r=!rsi { period: 2 }'` scopes fine,
  because what follows the colon assigns and a scopeless body would not.
...
| `kind` | `market`, `stop`, `take_profit`, `limit`, or `liquidation` — the last being a forced close after the account breached `--maintenance-margin`, i.e. the wallet acting rather than the strategy. |
