...
- `talib_expected.csv` — **generated, and not committed**. One column of TA-Lib
  output per indicator, aligned row-for-row with the input. Because it is
  absent from a clean checkout, `tests/talib_validation.rs` **skips there** —
  see *The skip is opt-out* below.
...
from a pass**, so a cross-check can rot for months without a red build.
`doc/CONTRIBUTING.md` lists `talib_validation.rs` as a drift guard, and on a
clean checkout it guards nothing.

So set `FUGAZI_REQUIRE_FIXTURES=1` to turn every missing-or-stale fixture from a
skip into a failure:
...
That is the mode for a CI job that provisions the reference libraries, and the
way to prove locally that a suite really compared rather than returning early.
