//! Market-data vocabulary: the [`Real`] scalar every price and indicator output
//! flows through, the OHLCV [`Candle`], the shared per-run [`Schema`] and its
//! per-bar [`OverlayInfo`] values (with the runtime carrier [`OverlayValue`]
//! and its type tag [`OverlayType`]), and the composite per-bar input
//! [`Atom`] that the [`Indicator`](crate::Indicator) chain consumes.
