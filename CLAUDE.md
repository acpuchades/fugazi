...
| Persist / resume a run's full state | `RunnableStrategy::{save_state, restore_state, drive_resumable}` + `RunState`; `backtest::{run_iteration_resumable, apply_closeout}`. See ARCHITECTURE *Run resuming* | `src/spec/{runnable,backtest}.rs`, `src/backtest.rs` |
