...
and did not get one, and charged nothing. The `run` warning covers a column that
is wholly absent; a column with **holes** shows up as `carry_coverage()`
reporting fewer seen than wanted.
